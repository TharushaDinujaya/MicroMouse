...
// Maze geometry related constants
...
// data structures related constants
...
// struct for represent the distance metrix of the mouse
...
// struct for represent the orientation of the robot
enum Orient
{
...
void rotate180(int leftDistance, int rightDistance);

// queue function headers and global variables
...
// motion related high level functions headers
void rotateMouse(int direction);
void mouseForward();
void blinkLED(int times);
DistanceMetrix loadDistances();

// overall algorith related function headers
void searchRun();
void returnToStart();
void test();
void followTest();
...
void loop()
{
	...
	for (int i = 0; i < ROUNDS; i++)
	{
		...
		finalX = 0;
		finalY = 0;
		returnRun = true; // set the return run flag to true

		searchRun();

		DistanceMetrix dt = loadDistances();
		rotate180(dt.leftFront, dt.rightFront);
	}
...
void rotate180(int leftD, int rightD)
{
	stop();
	right(120);
...
void resetFloodMap()
{
	...
	if (!returnRun)
	{
		...
		floodMap[finalX][finalY] = 0;
		floodMap[finalX][finalY + 1] = 0;
		floodMap[finalX + 1][finalY] = 0;
		floodMap[finalX + 1][finalY + 1] = 0;
	}
	else
	{
		floodMap[0][0] = 0;
...
void flooded()
{
	...
	if (!returnRun)
	{
		...
		enqueue({finalX, finalY});
		enqueue({finalX, finalY + 1});
		enqueue({finalX + 1, finalY});
		enqueue({finalX + 1, finalY + 1});
	}
	else
	{
		enqueue({0, 0});
...
bool isFinished(Point current)
{
	...
	if (!returnRun)
	{
		return (x == finalX || x == finalX + 1) && (y == finalY || y == finalY + 1);
	}
	return x == 0 && y == 0;
}
...
// function for populate the distances from the sensors
DistanceMetrix loadDistances()
{
	...
	// fetch the distance from the sensor
	DistanceMetrix dt;
	dt.front = measure4.RangeMilliMeter;
	dt.leftFront = measure1.RangeMilliMeter;
	dt.rightFront = measure3.RangeMilliMeter;
	dt.leftBack = measure2.RangeMilliMeter;
	dt.rightBack = measure5.RangeMilliMeter;

	return dt;
}

// function for blink the LED
void blinkLED(int times)
{
